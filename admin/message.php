...
const PAGE_TITLE = "Message - Escrow";
        ...
        <div class="container-fluid py-4">
            <div class="container mt-4">
                <div class="card">
                    ...
                    <div class="card-footer d-flex justify-content-between align-items-center">
                        <div class='d-flex flex-column'>
                            <p>
                                ...
                                <?php
                                $select_buyer_status = "SELECT * FROM payment_request WHERE transaction_id='$transaction_id' AND user_id='" . $get_transaction['buyer'] . "'";
                                ...
                                ?>
                                    ...
                                    $get_buyer_status = mysqli_fetch_assoc($query_buyer_status);
                                    if ($get_buyer_status['status'] == 'not seen') {
                                    ?>
                            <form action="" method="post">
                                <input type="hidden" name="payment_id" value="<?= $get_buyer_status['id'] ?>">
                                <button class="btn btn-success" type="submit">Mark as Seen</button>
                            </form>
                        <?php
                                    } else {
                        ?>
                    ...
                    ?>
                        ...
                        <!-- check if seller and buyer are both seen -->
                        <?php
                        $select_both_status = "SELECT * FROM payment_request WHERE transaction_id='$transaction_id' AND (user_id='" . $get_transaction['seller'] . "' OR user_id='" . $get_transaction['buyer'] . "') AND status='seen'";
                        $query_both_status = mysqli_query($con, $select_both_status);
                        if (mysqli_num_rows($query_both_status) == 2) {
                            if ($get_transaction['state'] == 'pending') {

                        ?>

                                <a href="func/mark_transaction_complete?transaction=<?= $transaction_id; ?>" class="btn btn-success" style="height: 2.5rem;">Mark Transaction Complete</a>
                        <?php
                            }
                        }
                        ?>
                        ...
                        <div class='d-flex flex-column'>
                            <p>
                                ...
                                <?php
                                $select_seller_status = "SELECT * FROM payment_request WHERE transaction_id='$transaction_id' AND user_id='" . $get_transaction['seller'] . "'";
                                ...
                                ?>
                                    ...
                                    $get_seller_status = mysqli_fetch_assoc($query_seller_status);
                                    if ($get_seller_status['status'] == 'not seen') {
                                    ?>
                            <form action="" method="post">
                                <input type="hidden" name="payment_id" value="<?= $get_seller_status['id'] ?>">
                                <button class="btn btn-success" type="submit">Mark as Seen</button>
                            </form>
                        <?php
                                    } else {
                    ...
                    ?>
                        ...
                        <!-- <p>Seller Status<br /><strong class="text-warning">not yet sent</strong></p> -->
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="walletDialog" tabindex="-1" aria-labelledby="walletDialogLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="walletDialogLabel">Admin Wallet Address</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <label for="walletAddress" class="form-label">Wallet Address:</label>
                            <div class="input-group">
                                <input type="text" id="walletAddress" class="form-control" value="0x123456789ABCDEF" readonly>
